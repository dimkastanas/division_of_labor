...
Precision-weighted RSA model of referential over-specification (see precision_rsa.qmd).
    ...
    I_mix    = occ*U_asym + (1-occ)*U_ego          # w_S fixed at 1

sigma_f = sigmoid(s_f): per-feature salience. lam: informativity. beta: brevity cost
(the group/autism effect). w_S is freed per group only in the M_wS rival (section 3).
"""
...
def speaker(s5, lam, beta, occ, dv):
    """P over the 81 utterances for one condition (occ in {0,1}, dv = distvec).
    P(u) ∝ P_Sal(u) · L0(target|u)^lam · exp(-beta*|u|).
    Occlusion triggers a full perspective switch (w_S fixed at 1)."""
    ...
    I_mix = occ * U_asym + (1.0 - occ) * U_ego                 # full switch under occlusion
    return jax.nn.softmax(logP_Sal + lam * I_mix - beta * NCOST)


# vmap over trials (per-trial beta / occ / distvec); s5, lam shared
spk = jax.vmap(speaker, in_axes=(None, None, 0, 0, 0))


def mention_prob(s5, lam, beta, dv, occ=0.0):
    """P(mention each of the 8 features) for one condition."""
    return speaker(s5, lam, beta, occ, dv) @ MENTION
...
def unpack(theta, ncells):
    ...
    beta = theta[6:6 + ncells]
    return s5, lam, beta


def nll(theta, cell, occ, dv, uidx, ncells):
    s5, lam, beta = unpack(theta, ncells)
    P = spk(s5, lam, beta[cell], occ.astype(jnp.float64), dv)
...
def fit(cell, occ, dv, uidx, ncells, theta0=None):
    ...
    s5, lam, beta = unpack(jnp.array(res.x), ncells)
    k = 4 + 1 + ncells                              # salience(4, centred) + lam + beta
    sigma5 = np.asarray(jax.nn.sigmoid(s5 - jnp.mean(s5)))      # intrinsic salience per dim
    return dict(s5=np.asarray(s5), sigma=sigma5, lam=float(lam),
...
def recovery(cell, occ, dv, ncells, true_beta, labels, seed=0):
    ...
    theta = np.concatenate([s_true, [float(np.log(np.expm1(6.0)))], list(true_beta)])
